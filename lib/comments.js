...
async function getRepository(repo, token, proxyFetch) {
  const response = await proxyFetch(`${API_BASE}/repos/${repo.owner}/${repo.repo}`, {
    headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github.v3+json", "User-Agent": USER_AGENT },
  });
  if (response.status === 404) return null;
  if (!response.ok) throw new Error(`Failed to inspect repository: ${response.status}`);
  return response.json();
}

async function findPRForBranch(owner, repo, branch, token, proxyFetch, headRepo = null, allowMissing = false) {
  const headOwner = headRepo?.owner || owner;
  const url = `${API_BASE}/repos/${owner}/${repo}/pulls?head=${encodeURIComponent(`${headOwner}:${branch}`)}&state=open&per_page=100`;
  // An inaccessible candidate remote may coexist with the actual upstream.
  const fetchCandidate = async (url, options) => {
  ...
  const prs = await fetchAllPages(url, token, fetchCandidate);
  if (headRepo && prs.some((pr) => pr.head?.ref === branch && !pr.head.repo)) {
    throw new Error("A matching PR has a deleted or inaccessible head repository. Cannot verify its identity; use GH_REPO=owner/repo with --pr <number>.");
  }
  const matches = headRepo ? prs.filter((pr) =>
    pr.head?.ref === branch &&
    pr.head?.repo?.full_name?.toLowerCase() === `${headRepo.owner}/${headRepo.repo}`.toLowerCase()
  ) : prs;
  if (matches.length > 1) throw new Error("Multiple open PRs match this branch. Use GH_REPO=owner/repo and --pr <number> to select one.");
...
const DEFAULT_META_FILTERS = [
  // agent-reviews reply to a non-thread comment (posted as issue comment)
...
function filterComments(comments, options) {
  const ignored = new Set((options.ignoredAuthors || []).map((author) => author.toLowerCase()));
  let filtered = comments.filter((comment) => !ignored.has(comment.user?.toLowerCase()));
...
async function replyToComment(
  ...
  // Only inline review comments support threaded replies via this endpoint.
  // PR-level issue comments and review-summary submissions return 404 here.
  ...
  if (response.status === 404) {
    // Not an inline review comment, so there's no thread to reply into.
    // Skip rather than post a top-level "Re: comment X" issue comment,
    // which is noisy and not actually a reply.
    return {
      skipped: true,
      reason:
        "comment is not part of a review thread (only inline review comments support threaded replies)",
    };
  }
