...
// Base URL for the GitHub API. Defaults to api.github.com.
// Set GITHUB_API_URL to point at a GitHub Enterprise host or a local
// API-compatible server (e.g., for testing or recording).
...
async function resolveThread(owner, repo, prNumber, commentId, token, proxyFetch) {
  ...
  while (!thread) {
    const response = await proxyFetch(`${API_BASE}/graphql`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  const resolveResponse = await proxyFetch(`${API_BASE}/graphql`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
